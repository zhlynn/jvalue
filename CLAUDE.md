...
- `make` — builds the `jvalue` demo binary into the repo root via `g++ -Wall -O2 -std=c++11 -Isrc`. Object files go into `.build/`.
- `make test` — builds the test suite binary (`.build/test`, 59 test cases).
- `make benchmark` — builds the performance benchmark binary (`.build/benchmark`).
- `make clean` — removes `.build/` and the `jvalue` binary.

Layout: the library is `src/json.{h,cpp}`; the demo `main.cpp` sits at the repo root; test and benchmark sources live in `test/`. All non-library sources compile with `-Isrc`.
...
### Base64 (`jbase64`, in src/json.{h,cpp})

`jbase64` provides encode/decode used by `jvalue::assign_data` / `as_data` and the plist data-tag path. It is declared at the end of `json.h` and implemented at the end of `json.cpp` (the library is fully contained in those two files). It caches allocated buffers in `m_array_encodes` / `m_array_decodes` and frees them in the destructor — callers hold pointers only for the lifetime of the `jbase64` instance.

### Test suite and benchmark

- `test/test.cpp` — 59 test cases covering construction, copy/move, JSON parse/write, plist roundtrip, object/array operations, type conversions, edge cases. Run via `make test && ./.build/test`.
- `test/benchmark.cpp` — large-scale performance benchmark (200-user nested JSON, 2000-element arrays, 100/500-device plists with binary data and dates). Run via `make benchmark && ./.build/benchmark`.
